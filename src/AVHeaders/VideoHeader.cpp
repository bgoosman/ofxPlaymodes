...
//------------------------------------------------------
    void VideoHeader::draw() {
        ...
        ofDrawRectangle(currentLength + PMDRAWSPACING - oneLength, PMDRAWELEMENTSY + 30, oneLength, 10);
...
//------------------------------------------------------
    ...
    void VideoHeader::setCurrentFrame(int index) {
        this->currentFrameIndex = index;
    }

    VideoFrame VideoHeader::getCurrentVideoFrame() {
        return buffer->getVideoFrame(currentFrameIndex);
    }
            ...
            case 0 :
                ...
                if (playing && (int(position) > (outAbsFrame))) {
                    if (loopMode == OF_LOOP_NORMAL) position = double(inAbsFrame);
                        ...
                        std::cout << "reaching outpoint: setPlaying false" << std::endl;
                        setPlaying(false);
                    ...
                    if (loopMode == OF_LOOP_NORMAL) position = double(outAbsFrame);
                        ...
                        std::cout << "reaching inpoint: setPlaying false" << std::endl;
                        setPlaying(false);
                    } else if (loopMode == OF_LOOP_PALINDROME) {
                        speed = -speed;
                    }
                ...
                if (playing) {
                    position = CLAMP(position, double(inAbsFrame), double(outAbsFrame));
                } else {
                    position = CLAMP(position, double(lastAbsFrame), double(totalNumFr));
                }
...
//------------------------------------------------------
    ...
    void VideoHeader::pause() {
        isPaused = true;
    }

    void VideoHeader::resume() {
        isPaused = false;
    }
...
//------------------------------------------------------
    void VideoHeader::setDelayMs(float delayMs) {
        double oneFrame = (TimeDiff) (1000000.0 / fps / 1.0); // 33333
        int delayToSet = int(double(delayMs * 1000.0)); // 5000000
